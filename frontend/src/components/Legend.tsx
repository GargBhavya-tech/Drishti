...
const TERRAIN_KEY: { label: string; color: string }[] = [
  { label: "Drivable terrain", color: CLASS_COLOR[1] },
  { label: "Vegetation", color: CLASS_COLOR[5] },
  { label: "Obstacle (rock/debris)", color: CLASS_COLOR[4] },
  { label: "Unknown (unmapped)", color: CLASS_COLOR[0] },
]

export function Legend() {
  return (
    <div className="panel-light p-3 pointer-events-auto">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex items-center gap-2.5 pt-1 mt-1 border-t border-[#D9E2EC]">
          <span className="h-2.5 w-2.5 rounded-sm flex-none" style={{ background: HAZARD_COLOR }} />
          <span className="text-[13px] text-[#1F2933]">Hazard (trench/ditch)</span>
        </div>
        <div className="flex items-center gap-2.5">
          <span className="h-[2px] w-3 flex-none" style={{ background: PATH_COLOR }} />
          <span className="text-[13px] text-[#1F2933]">Planned safe path</span>
        </div>
      </div>
    </div>
  )
